---
title: Additional Resources for Outreach, Training, & Certification
layout: layouts/resource-index
eleventyNavigation:
  order: 8
  title: Additional Resources
items:
  - heading: Handouts
    items:
      - name: Sample Memo for Annual Notification (FAA)
        url: /~assets/docs/sample-memo-for-annual-notification-faa.docx
      - name: Your Rights as a Whistleblower for a Government Corporation (v2024)
        url: /~assets/docs/your-rights-as-a-whistleblower-for-a-government-corporation-v2024.pdf
      - name: Your Rights as an Employee of a Government Corporation (v2024)
        url: /~assets/docs/your-rights-as-an-employee-of-a-government-corporation-v2024.pdf
      - name: Your Rights as a Federal Employee (v2024)
        url: /~assets/docs/your-rights-as-a-federal-employee-v2024.pdf
      - name: Discrimination Fact Sheet (b)(1)
        url: /~assets/docs/discrimination-fact-sheet-b1.pdf
      - name: Discrimination Fact Sheet (b)(10)
        url: /~assets/docs/discrimination-fact-sheet-b10.pdf
      - name: Hiring Offense Fact Sheet (b)(2), Improper Recommendation
        url: /~assets/docs/hiring-offense-fact-sheet-b2-improper-recommendation.pdf
      - name: Hiring Offense Fact Sheet (b)(4), Obstruction of Competition
        url: /~assets/docs/hiring-offense-fact-sheet-b4-obstruction-of-competition.pdf
      - name: Hiring Offense Fact Sheet (b)(5), Influencing Withdrawal
        url: /~assets/docs/hiring-offense-fact-sheet-b5-influencing-withdrawal.pdf
      - name: Hiring Offense Fact Sheet (b)(6), Unauthorized Preference
        url: /~assets/docs/hiring-offense-fact-sheet-b6-unauthorized-preference.pdf
      - name: Hiring Offense Fact Sheet (b)(7), Nepotism
        url: /~assets/docs/hiring-offense-fact-sheet-b7-nepotism.pdf
      - name: Hiring Offense Fact Sheet (b)(11), Violating Veterans Preference
        url: /~assets/docs/hiring-offense-fact-sheet-b11-violating-veterans-preference.pdf
      - name: Potpourri Fact Sheet (b)(13), Nondisclosure Forms, Policies, and Agreements
        url: /~assets/docs/potpourri-fact-sheet-b13-nondisclosure-forms-policies-and-agreements.pdf
      - name: Potpourri Fact Sheet (b)(12), Violating Merit System Principles
        url: /~assets/docs/potpourri-fact-sheet-b12-violating-merit-system-principles.pdf
      - name: Potpourri Fact Sheet (b)(3), Coercing Political Activity
        url: /~assets/docs/potpourri-fact-sheet-b3-coercing-political-activity.pdf
      - name: Potpourri Fact Sheet (b)(14), Accessing Medical Records
        url: /~assets/docs/potpourri-fact-sheet-b14-accessing-medical-records.pdf
      - name: Retaliation Fact Sheet (b)(9), Protected Activity
        url: /~assets/docs/retaliation-fact-sheet-b9-protected-activity.pdf
      - name: Retaliation Fact Sheet (b)(8), Protected Disclosure
        url: /~assets/docs/retaliation-fact-sheet-b8-protected-disclosure.pdf
  - heading: Posters
    items:
      - name: The Hatch Act and Further Restricted Employees Poster
        url: /~assets/docs/the-hatch-act-and-further-restricted-employees-poster.pdf
      - name: The Hatch Act and State, D.C., and Local Employees Poster
        url: /~assets/docs/the-hatch-act-and-state-dc-and-local-employees-poster.pdf
      - name: Hatch Act Social Media Handout
        url: /~assets/docs/hatch-act-social-media-handout.pdf
      - name: Disclosures of Wrongdoing Poster (v2024)
        url: /~assets/docs/disclosures-of-wrongdoing-poster-v2024.pdf
      - name: Prohibited Personnel Practices Poster (v2024)
        url: /~assets/docs/prohibited-personnel-practices-poster-v2024.pdf
      - name: Prohibited Personnel Practices Poster for Government Corporations (v2024)
        url: /~assets/docs/prohibited-personnel-practices-poster-for-government-corporation-v2024.pdf
      - name: The Hatch Act and Most Federal Employees Poster
        url: /~assets/docs/the-hatch-act-and-most-federal-employees-poster.pdf
      - name: Whistleblower Retaliation Poster (v2024)
        url: /~assets/docs/whistleblower-retaliation-poster-v2024.pdf
      - name: 2025 OSC EEO Poster (Letter Format)
        url: /~assets/docs/2025-osc-eeo-poster-letter-format.pdf
  - heading: Training Materials
    items:
      - name: The Unique Role of OSC in Protecting LGBT Rights in the Federal Workplace
        url: /~assets/docs/the-unique-role-of-osc-in-protecting-lgbt-rights-in-the-federal-workplace.pdf
      - name: FY25 Agency-Conducted Annual Supervisory Training
        url: /~assets/docs/fy25-agency-conducted-annual-supervisory-training.pdf
  - heading: Equal Employment Opportunity Resources
    items:
      - name: No FEAR Act Quarterly Data Q3 2025
        url: /~assets/docs/no-fear-act-quarterly-data-q3-2025.pdf
      - name: Federal Protections for Lesbian, Gay, Bisexual, and Transgender (LGBT) Individuals
        url: /~assets/docs/deral-protections-for-lesbian-gay-bisexual-and-transgender-lgb-51da6d07.pdf
      - name: No FEAR Act PowerPoint Presentation
        url: /~assets/docs/no-fear-act-powerpoint-presentation.ppt
      - name: OSC's Plan for Improving Access for Persons with Limited English Proficiency
        url: /~assets/docs/oscs-plan-for-improving-access-for-persons-with-limited-english-proficiency.pdf
      - name: MD-715 AAP Report (FY24)
        url: /~assets/docs/md715-aap-report-fy24.pdf
---
